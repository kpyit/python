...
class load_component():
    ...
    def __init__(self): 
        ...
        self.static_canvas = FigureCanvas(Figure(figsize=(3, 3), tight_layout=True))
    ...
    def draw_avch_current_diode(self, diode):
        ...
        self._static_ax.plot(points_y,points_x,'o',label="заданные точки", color='red')
        self._static_ax.plot(ynew,xnew,label="расчетная кривая", color='green')
        self._static_ax.set_title(f'Расчетная характеристика диода {diode[imp_l.POS_NAME]} по точкам')
        ...
        self._static_ax.grid(b=True, which='major', color='#777777', linestyle='-', alpha=0.2) #https://stackoverflow.com/questions/60020071/why-is-the-grid-turned-on-only-on-the-last-subplot
    ...
    def table_insert_diodes(self, list_diodes:list):
        ...
        for diode in list_diodes:
            self.table.setItem(row, 0, QtWidgets.QTableWidgetItem(diode[imp_l.POS_NAME]))
            self.table.setItem(row, 1, QtWidgets.QTableWidgetItem(TYPE_DIODE[diode[imp_l.POS_TYPE]]))
            self.table.setItem(row, 2, QtWidgets.QTableWidgetItem(str(diode[imp_l.POS_V])))
            self.table.setItem(row, 3, QtWidgets.QTableWidgetItem(str(diode[imp_l.POS_A])))
            self.table.setItem(row, 4, QtWidgets.QTableWidgetItem(BODY_TYPE[diode[imp_l.POS_TYPE_B]]))
            self.table.setItem(row, 5, QtWidgets.QTableWidgetItem(str(diode[imp_l.POS_POINTS])))
            self.table.setItem(row, 6, QtWidgets.QTableWidgetItem(str(diode[imp_l.POS_DATASHEET])))
            row=row+1 
            ...
            with open(PATH_IMPORT+FILE_IMPORT_DIODES_TEXT, mode="r", encoding="utf-8") as f:
                for line in f.readlines():
                    if not line.startswith("#"):#
                        ...
                        #
                        coef = math_mc.calc_coef_regression(diode[imp_l.POS_POINTS])
